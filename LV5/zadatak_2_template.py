...
# a)
df.groupby('species').size().plot(kind='bar', color='green', edgecolor='black')
...
# f)
ohe = OneHotEncoder()
X_encoded = ohe.fit_transform(df[["island"]]).toarray()
...
LogRegression_model_moreInputs = LogisticRegression()
LogRegression_model_moreInputs.fit(X_train, y_train)

y_test_p = LogRegression_model_moreInputs.predict(X_test)
